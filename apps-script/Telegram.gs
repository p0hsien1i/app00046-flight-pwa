...
function cronHourly() {
  try {
    ...
    flights.forEach(function (f) {
      ...
      var title = f.flight_no + " " + f.dep_iata + "→" + f.arr_iata;
      ...
      if (now >= depMs - 24 * 3600e3 && now < depMs - 12 * 3600e3 && !notifSent_(f.id + ":checkin")) {
        var msg1 = "✅ <b>Check-in opens</b> — " + title +
          "\nDeparts " + fmtLocal_(f.dep_time_local) + " (" + f.dep_tz + ")" +
          (f.pnr ? "\nPNR: <code>" + f.pnr + "</code>" : "");
        sendTelegram_(msg1);
        markNotif_(f.id + ":checkin", msg1);
      }

      // 2) day-of summary (from 07:00 local at departure airport)
      var localNow = Utilities.formatDate(new Date(), f.dep_tz, "yyyy-MM-dd HH");
      var depLocalDate = String(f.dep_time_local).slice(0, 10);
      if (localNow.slice(0, 10) === depLocalDate && Number(localNow.slice(11)) >= 7 &&
          now < depMs && !notifSent_(f.id + ":dayof")) {
        var parts = [
          "🛫 <b>Flight day</b> — " + title,
          "Departs " + fmtLocal_(f.dep_time_local) + " (" + f.dep_tz + ")",
        ];
        if (f.dep_terminal) parts.push("Terminal " + f.dep_terminal + (f.dep_gate ? " · Gate " + f.dep_gate : ""));
        if (f.seat) parts.push("Seat " + f.seat);
        if (f.pnr) parts.push("PNR <code>" + f.pnr + "</code>");
        var msg2 = parts.join("\n");
        sendTelegram_(msg2);
        markNotif_(f.id + ":dayof", msg2);
      }
      ...
      if (now >= depMs - 48 * 3600e3 && now <= depMs + 2 * 3600e3 &&
        ...
        var res = flightInfo_(f.flight_no, depLocalDate, false);
